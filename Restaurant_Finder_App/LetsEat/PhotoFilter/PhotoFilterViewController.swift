...
import UIKit
import AVFoundation
//The AVFoundation framework contains methods for capturing, processing, synthesizing, controlling, importing, and exporting audiovisual media on Apple platforms.
import CoreServices
// The CoreServices framework contains programming interfaces for working with system-declared uniform types.
...
private extension PhotoFilterViewController {
    ...
    func setupCollectionView() {
        ...
        layout.sectionInset = UIEdgeInsets(top: 7, left: 7,
                                           bottom: 7, right: 7)
...
extension PhotoFilterViewController: ImageFiltering {
    ...
    func filterSelected(item: FilterItem) {
        if let img = image {
            if item.filter != "None" {
                imgExample.image = self.apply(filter: item.filter,
                                              originalImage: img)
            } else {
